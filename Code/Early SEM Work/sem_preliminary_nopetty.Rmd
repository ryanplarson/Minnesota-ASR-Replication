...
monsanc.short %>%
  ...
  group_by(Race) %>%
  summarize(
    `Confinement Days`= mean(conf_days, na.rm = T),
  ) %>%
  gather(key = "variable", value= "value", -Race) %>%
...
mv.fe <- lm(cbind(total_ff_log, conf_days_log)~black+hispanic+asian+nativeam+other.race+
...
iv.fe <- ivreg(total_ff_log~conf_days_log+black+hispanic+asian+nativeam+other.race+male+log(age)+
                  priors+trial_flag+felony+gross.mis+
...
#FE OLS
fe.ols <- lm(total_ff_log~conf_days_log+
...
#IV F test
# First Stage
first_stage <- lm(conf_days_log~cap_ratio+black+hispanic+asian+nativeam+other.race+male+log(age)+
                  priors+trial_flag+felony+gross.mis+
                  drug+alcohol.dui+hunt.fish+other.offense+
                  as.factor(filed_county)+as.factor(sentence_year),
                  data=monsanc.short[monsanc.short$missiv==F,])
