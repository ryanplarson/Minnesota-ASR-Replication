...
stargazer(as.data.frame(felony[,c(23,54,56,116,105,106,107,108,109,110,53,111,112,113,114,115,97,80,94)]), 
...
```{r, include=F}
corr<-round(cor(felony[,c(96,54,56,116,105,106,107,108,109,110,53,111,112,113,114,115,97,80,94)], use="pairwise.complete.obs", method="pearson" ),2)
corr[upper.tri(corr)] <-""
...
m33f <- lmer(total_ff_log ~ black+hispanic+asian+nativeam+other.race+male+log(age)+priors+trial_flag+
              drug+alcohol.dui+hunt.fish+other.offense+(1|filed_county), REML = F, data = felony)
class(m33f) <- "lmerMod" 

m34f <- lmer(total_ff_log ~ black+hispanic+asian+nativeam+other.race+male+log(age)+priors+trial_flag+
