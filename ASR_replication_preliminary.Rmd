...
felony <- felony %>%
  mutate(case_short = str_extract(str_sub(current_case_number,start=3), "[0-9]+"))

#joining and munging MNSG data for crim hist/severity measures
mnsg <- read_csv("~/MonSanc/msgc_crimhistory.csv") %>%
  select(case_number, severity_conv, chpoints_final)

felony <- felony %>% left_join(mnsg, by = c("case_short"="case_number"))
```
...
m33f <- lmer(total_ff_log ~ severity_conv+chpoints_final+
               ...
               trial_flag+drug+alcohol.dui+hunt.fish+other.offense+(1|filed_county), REML = F, data = felony)
class(m33f) <- "lmerMod" 
...
stargazer(m41f, m42f, m43f, m44f,
          ...
          covariate.labels = c("Severity Score (Conv.)", "Crim. Hist. Pts.", "Age", 
                               "Male",  "Black",  "Trial", 
